...
const run = async () => {
  fs.createReadStream('src/scripts/growing/8_new_users_team_member.csv')
    .pipe(csv())
    .on('data', row => {
      pool.getConnection((err, conn) => {
        ...
        conn.query(
          ...
          (err, result1) => {
            conn.query(
              // 2. 팀 조회
              `select id, leader_id from team where type = 'NEW' and name like ?`,
              // @ts-ignore
              [`${row.leader}%`],
              // 순모임
              (err, result2) => {
                ...
                conn.execute(
                  ...
                  [
                    ...
                    result1[0].id,
                    // @ts-ignore
                    result1[0].id === result2[0].leader_id ? 'LEADER' : 'NEW_COMER',
                  ],
                  (err, results, fields) => {
                    if (err) {
