...
const run = async () => {
  fs.createReadStream('src/scripts/growing/9_new_users_attendance.csv')
    .pipe(csv())
    .on('data', row => {
        ...
        conn.query(
          ...
          (err, result) => {
            ...
            Object.keys(row)
              ...
              .map(el => {
                return `insert into attendance(created_at, created_by, updated_at, updated_by, team_member_id, week, status, etc) value (now(), null, now(), null, ${el[0]}, '${el[1]}', '${el[2]}', null);`
              })
              .forEach(stmt => {
