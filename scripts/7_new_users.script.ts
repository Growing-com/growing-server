...
const run = async () => {
  fs.createReadStream('src/scripts/growing/7_new_users.csv')
    .pipe(csv())
    .on('data', row => {
      pool.getConnection((err, conn) => {
        ...
        conn.execute(
          ...
          `insert ignore into users (created_at, created_by, updated_at, updated_by, username, password, role, name, phone_number, sex, birth, grade, is_active, visit_date, visit_term_id)
                  values ('2023-09-01', null, '2023-09-01', null, '${generateObjectId()}', '${generateObjectId()}', 'NORMAL', ?, ?, ?, ?, ?, 1, '1970-01-01', 1)`,
          [row.name, row.phone, row.sex === '남' ? 'MALE' : 'FEMALE', '1970-01-01', row.grade],
          (err, results, fields) => {
            if (err) {
